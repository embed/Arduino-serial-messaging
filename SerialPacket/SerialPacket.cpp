...
boolean SerialPacket::parseSerialData()
{
    ...
    switch(incomingPacket.packetType)
    {
        ...
        case COMMAND_REPLY:
            incomingPacket.payload = hex_to_dec(_inputChar[10])*16 + hex_to_dec(_inputChar[11]);
            incomingPacket.commandID = hex_to_dec(_inputChar[7])*16 + hex_to_dec(_inputChar[8]);
            checkedParity = incomingPacket.packetType^incomingPacket.nodeID^incomingPacket.commandID^incomingPacket.payload;
            break;

        case COMMAND_WIDE:
            incomingPacket.commandID = hex_to_dec(_inputChar[7])*16 + hex_to_dec(_inputChar[8]);
            payloadH = hex_to_dec(_inputChar[10])*16 + hex_to_dec(_inputChar[11]);
            payloadL = hex_to_dec(_inputChar[12])*16 + hex_to_dec(_inputChar[13]);
            incomingPacket.payload = (payloadH << 8) | payloadL;
            checkedParity = incomingPacket.packetType^incomingPacket.nodeID^incomingPacket.commandID^payloadH^payloadL;
            break;    

        //TODO: Handling of DATA_INT is wrong!!!
        case DATA_BYTE:
        case DATA_REQUEST:
        case DATA_INT:
            incomingPacket.sensorID = hex_to_dec(_inputChar[7])*16 + hex_to_dec(_inputChar[8]);;
            incomingPacket.payload = hex_to_dec(_inputChar[10])*16 + hex_to_dec(_inputChar[11]);
            checkedParity = incomingPacket.packetType^incomingPacket.nodeID^incomingPacket.sensorID^incomingPacket.payload;
            break;
    ...
    else if ((incomingPacket.packetType == DATA_INT) ) {
        incomingPacket.sensorID = hex_to_dec(_inputChar[7])*16 + hex_to_dec(_inputChar[8]);;
        _checkedParity = incomingPacket.packetType^incomingPacket.nodeID^incomingPacket.sensorID^incomingPacket.payload;
    }
